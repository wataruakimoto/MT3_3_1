...
Vector3 GetWorldPos(const Matrix4x4& worldMatrix) {
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		shoulderSphere.center = GetWorldPos(shoulderWorldMatrix);
		shoulderSphere.radius = 0.1f;

		elbowSphere.center = GetWorldPos(elbowWorldMatrix);
		elbowSphere.radius = 0.1f;

		handSphere.center = GetWorldPos(handWorldMatrix);
		handSphere.radius = 0.1f;

		shoulderToElbow.origin = GetWorldPos(shoulderWorldMatrix);
		shoulderToElbow.diff = Subtract(GetWorldPos(elbowWorldMatrix), GetWorldPos(shoulderWorldMatrix));

		elbowToHand.origin = GetWorldPos(elbowWorldMatrix);
		elbowToHand.diff = Subtract(GetWorldPos(handWorldMatrix), GetWorldPos(elbowWorldMatrix));
